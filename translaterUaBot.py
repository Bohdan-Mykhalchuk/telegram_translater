...
@bot.message_handler(
    func=lambda message: message.text in ['English', 'Spanish', 'Portuguese', 'German', 'French', 'Italian'])
def send_welcome(message):
    ...
    bot.send_message(message.chat.id, f"{user_name.capitalize()}, please select which translater you want to use.",
                     reply_markup=markup)
...
def handle_translator(message):
    ...
    bot.reply_to(message,
                 f"Perfect! Now we are working with {message.text} translator. Please write what do you want to translate.",
...
def show_main_menu(*, message: Message) -> None:
    ...
    bot.send_message(message.chat.id,
                     f"{user_name.capitalize()}, welcome to Translater! Which language would you like to translate into?",
                     reply_markup=markup)
